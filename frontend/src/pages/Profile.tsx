...
const Profile: React.FC<ProfileProps> = ({ theme }) => {
  ...
  const [loadingPlatform, setLoadingPlatform] = useState<LoadingPlatform>({
    github: false,
    linkedin: false,
    facebook: false,
    instagram: false,
  ...
  const handleUpdateSocialMedia = async (platform: keyof SocialMedia) => {
  ...
  const formatLink = (platform: string, username: string) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {(
          ["github", "linkedin", "facebook", "instagram", "twitter"] as const
        ).map((platform) => (
          <Box
            sx={{
              display: "flex",
        ...
        >
          Thank you for exploring Let-AI-Handle today! 🚀
        </Typography>
